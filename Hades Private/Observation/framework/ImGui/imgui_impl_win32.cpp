...
{
    bool is = _IsWindowsVersionOrGreater(0x0A00, 0x0A00, 0);
    if (is)
    {
    ...
    bool is_1 = _IsWindowsVersionOrGreater(0x0603, 0x0603, 0);
    if (is_1)
    {
...
float ImGui_ImplWin32_GetDpiScaleForMonitor(void* monitor)
{
    ...
    bool is = _IsWindowsVersionOrGreater(0x0603, 0x0603, 0);
    if (is)
    {
		static HINSTANCE shcore_dll = ::LoadLibraryA("shcore.dll"); // Reference counted per-process
...
{
    bool is = _IsWindowsVersionOrGreater(0x0600, 0x0600, 0);
    if (!is)
        return;
    ...
    BOOL opaque;
    DWORD color;
    bool is_1 = _IsWindowsVersionOrGreater(0x0602, 0x0602, 0);
    if (is_1 || (SUCCEEDED(::DwmGetColorizationColor(&color, &opaque)) && !opaque))
    {
